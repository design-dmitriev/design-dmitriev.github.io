document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.innerWidth <= 768) {
        // Набор цифр должен покрывать ВСЕ года из разметки: недостающая цифра
        // молча подменялась нулём (glyphs['0']), из-за чего 2023 на мобиле
        // рисовался как «2020». Держать полный 0-9 надёжнее, чем ровно те
        // цифры, что нужны сегодня.
        const glyphs = {
            '0': [" ██████╗ ", "██╔═████╗", "██║██╔██║", "████╔╝██║", "╚██████╔╝", " ╚═════╝ "],
            '1': [" ██╗", "███║", "╚██║", " ██║", " ██║", " ╚═╝"],
            '2': ["██████╗ ", "╚════██╗", " █████╔╝", "██╔═══╝ ", "███████╗", "╚══════╝"],
    ...
    function createVolumetricAscii(text) {
        ...
        const colors = [];
        const indicesArr = [];
        // Основной зелёный сайта (--hero-accent, #60b756). Раньше здесь был
        // отдельный #4ADE80 — цифры выпадали из палитры остального интерфейса.
        let baseR = 96 / 255, baseG = 183 / 255, baseB = 86 / 255;
        let shaderLogic = ``;
